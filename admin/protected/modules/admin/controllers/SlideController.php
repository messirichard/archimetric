...
class SlideController extends ControllerAdmin
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Slide']))
		{
			...
			// box black fcs
			$picture_text = CUploadedFile::getInstance($model,'picture_text');
			if ($picture_text->name != '') {
				$model->picture_text = substr(md5(time()),0,5).'-'.$picture_text->name;
			}
			...
			if($model->validate() AND $valid){
				...
				try
				{
					...
					if ($picture_text->name != '') {
						$picture_text->saveAs(Yii::getPathOfAlias('webroot').'/images/slide/'.$model->picture_text);
					}
	...
	public function actionDelete($id)
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			@unlink(Yii::getPathOfAlias('webroot').'/images/slide/'.$data->image);
			$data->delete();
