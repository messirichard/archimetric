...
class GalleryController extends ControllerAdmin
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Gallery']))
		{
			...
			// $image_nutrition = CUploadedFile::getInstance($model,'image_nutrition');
			// $model->image_nutrition = substr(md5(time()),0,5).'-'.$image_nutrition->name;

			// $image_background = CUploadedFile::getInstance($model,'image_background');
			// $model->image_background = substr(md5(time()),0,5).'-'.$image_background->name;
			...
			if($model->validate() AND $valid){
				...
				try
				{
					...
					// $image_nutrition->saveAs(Yii::getPathOfAlias('webroot').'/images/gallery/'.$model->image_nutrition);
					// $image_background->saveAs(Yii::getPathOfAlias('webroot').'/images/gallery/'.$model->image_background);
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Gallery']))
		{
			...
			// $image_nutrition = CUploadedFile::getInstance($model,'image_nutrition');
			// if ($image_nutrition->name != '') {
			// 	$model->image_nutrition = substr(md5(time()),0,5).'-'.$image_nutrition->name;
			// }

			// $image_background = CUploadedFile::getInstance($model,'image_background');
			// if ($image_background->name != '') {
			// 	$model->image_background = substr(md5(time()),0,5).'-'.$image_background->name;
			// }
